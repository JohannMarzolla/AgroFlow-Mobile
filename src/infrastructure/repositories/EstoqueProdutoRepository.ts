...
export class EstoqueProdutoRepository implements IEstoqueProdutoRepository {
  async getAll(userId: string): Promise<EstoqueProduto[]> {
    try {
      const estoqueProdutoRef = collection(db, "users", userId, "EstoqueProduto");
      ...
      throw new Error("Erro ao buscar estoque produtos: " + (error as Error).message);
    }
  }

  async insert(userId: string, estoqueProduto: EstoqueProdutoAdicionarForm): Promise<void> {
    ...
    try {
      ...
      throw new Error("Erro ao inserir estoque produto: " + (error as Error).message);
    }
  }
} 